...
| 15 | **npcap-sdk ikilileri git geçmişinde** | HEAD'den çıkarıldı (#13), ama `fe556b8` ve öncesi commit'lerde `wpcap.lib` / `Packet.lib` duruyor. **2026-07-30 kararı: şimdilik dokunulmuyor.** Depo private'a çekildiği için yeniden dağıtım yüzeyi kapanıyor; `git filter-repo --path npcap-sdk --invert-paths` **destructive** ve force-push gerektiriyor. Depo tekrar herkese açılırsa bu madde yeniden kritik olur. ⚠️ Depo 2026-07-30'a kadar **public**'ti — bu ikililer o süre boyunca herkese açıktı, klonlanmış olabilirler. | `git log --all --oneline -- npcap-sdk \| head` |
| 16 | **MIT dönemi geri alınamaz** | Depo 2026-07-30'a kadar public ve MIT lisanslıydı. O tarihe kadarki revizyonlar, onları edinen herkes için MIT altında kullanılabilir kalır — lisans değişikliği yalnızca **bu revizyon ve sonrası** için geçerli. `LICENSE` §8 bunu açıkça söylüyor; iddia edilmemesi gereken şey "netscope hiç açık kaynak olmadı" cümlesidir. | `LICENSE` §8 |
