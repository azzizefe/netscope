...
use crate::registry::TransportClass;
...
impl Transport {
    /// The flow bucket for a protocol. Derived from the protocol registry,
    /// which is the single place a protocol's transport is declared.
    pub fn from_protocol(proto: &Protocol) -> Self {
        match proto.transport_class() {
            TransportClass::Tcp => Transport::Tcp,
            TransportClass::Udp => Transport::Udp,
            TransportClass::Icmp => Transport::Icmp,
            TransportClass::Arp => Transport::Arp,
            TransportClass::Other => Transport::Other,
        }
    }
}
...
/// How specific a protocol is as a label for the whole flow — a flow adopts
/// its highest-ranked packet's protocol. Declared per-protocol in the registry.
