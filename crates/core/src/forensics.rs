...
pub fn reconstruct_tcp_streams(packets: &[Packet]) -> HashMap<String, ReconstructedSession> {
    ...
    for pkt in packets {
        if pkt.protocol != Protocol::Tcp && pkt.protocol != Protocol::Http && pkt.protocol != Protocol::Tls {
            continue;
        }
        ...
        if client_flows.contains_key(&flow_key_forward) || (!client_flows.contains_key(&flow_key_reverse) && port_src > port_dst) {
            client_flows.entry(flow_key_forward).or_default().insert(seq, tcp_payload);
        } else {
            server_flows.entry(flow_key_reverse).or_default().insert(seq, tcp_payload);
        }
    }
    ...
    for (key, client_map) in client_flows {
        ...
        sessions.insert(key, ReconstructedSession {
            client_to_server,
            server_to_client,
        });
    }
...
pub fn carve_files(data: &[u8]) -> Vec<CarvedFile> {
    ...
    while idx < data.len() {
            ...
            if size > 10 && size <= 5_000_000 {
                ...
                for k in 2..(carved_data.len() - 4) {
                    if &carved_data[k..k+2] == b"\xFF\xFE" {
                        let len = ((carved_data[k+2] as usize) << 8) | (carved_data[k+3] as usize);
                ...
                if let Some(t_idx) = data_str.find("/Title") {
                    if let Some(open) = data_str[t_idx..].find('(') {
                        if let Some(close) = data_str[t_idx + open..].find(')') {
                            metadata.insert("Title".to_string(), data_str[t_idx+open+1 .. t_idx+open+close].to_string());
                        }
                    }
                }
                if let Some(a_idx) = data_str.find("/Author") {
                    if let Some(open) = data_str[a_idx..].find('(') {
                        if let Some(close) = data_str[a_idx + open..].find(')') {
                            metadata.insert("Author".to_string(), data_str[a_idx+open+1 .. a_idx+open+close].to_string());
                        }
                    }
                }
        ...
        if idx + 2 <= data.len() && &data[idx..idx+2] == b"MZ" {
            let mut is_pe = false;
            if idx + 64 <= data.len() {
                let pe_offset = u32::from_le_bytes(data[idx+0x3C .. idx+0x40].try_into().unwrap_or([0; 4])) as usize;
                if idx + pe_offset + 4 <= data.len() && &data[idx+pe_offset .. idx+pe_offset+4] == b"PE\x00\x00" {
                    is_pe = true;
                }
            }
            if is_pe {
...
pub fn build_timeline(packets: &[Packet]) -> Vec<TimelineEvent> {
    ...
    for p in packets {
        events.push(TimelineEvent {
            ...
            src: p.src_addr.map(|ip| ip.to_string()).unwrap_or_else(|| "-".into()),
            dst: p.dst_addr.map(|ip| ip.to_string()).unwrap_or_else(|| "-".into()),
...
mod tests {
    ...
    fn test_carve_png() {
        ...
        assert_eq!(carved[0].metadata.get("Dimensions").map(|s| s.as_str()), Some("256x256"));
    }
    ...
    fn test_carve_pdf() {
        ...
        assert_eq!(carved[0].metadata.get("Title").map(|s| s.as_str()), Some("Incident Report"));
        assert_eq!(carved[0].metadata.get("Author").map(|s| s.as_str()), Some("Analyst"));
    }
    ...
    fn test_timeline_export() {
        ...
        assert!(csv.contains("2026-07-15T18:30:00Z,10.0.0.1,8.8.8.8,Dns,64,\"Standard query A google.com\""));
    }
}
