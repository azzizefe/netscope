...
pub fn try_dissect(
    ...
    let msg_type = u32::from_le_bytes([ntlm_payload[8], ntlm_payload[9], ntlm_payload[10], ntlm_payload[11]]);
    let summary = match msg_type {
        1 => {
            ...
            if ntlm_payload.len() >= 16 {
                let flags = u32::from_le_bytes([ntlm_payload[12], ntlm_payload[13], ntlm_payload[14], ntlm_payload[15]]);
                ...
                if flags & 0x00001000 != 0 && ntlm_payload.len() >= 24 {
                    ...
                    let dom_offset = u32::from_le_bytes([ntlm_payload[20], ntlm_payload[21], ntlm_payload[22], ntlm_payload[23]]) as usize;
                    if dom_offset + dom_len <= ntlm_payload.len() {
                        domain = String::from_utf8_lossy(&ntlm_payload[dom_offset..dom_offset + dom_len]).into_owned();
                    }
                }
                // Read workstation if present
                if flags & 0x00002000 != 0 && ntlm_payload.len() >= 32 {
                    let work_len = u16::from_le_bytes([ntlm_payload[24], ntlm_payload[25]]) as usize;
                    let work_offset = u32::from_le_bytes([ntlm_payload[28], ntlm_payload[29], ntlm_payload[30], ntlm_payload[31]]) as usize;
                    if work_offset + work_len <= ntlm_payload.len() {
                        work = String::from_utf8_lossy(&ntlm_payload[work_offset..work_offset + work_len]).into_owned();
                    }
                }
            }
        ...
        2 => {
            ...
            if ntlm_payload.len() >= 20 {
                ...
                let target_offset = u32::from_le_bytes([ntlm_payload[16], ntlm_payload[17], ntlm_payload[18], ntlm_payload[19]]) as usize;
                if target_offset + target_len <= ntlm_payload.len() {
                    target = String::from_utf8_lossy(&ntlm_payload[target_offset..target_offset + target_len]).into_owned();
                }
            }
        ...
        3 => {
            ...
            if ntlm_payload.len() >= 64 {
                ...
                let dom_offset = u32::from_le_bytes([ntlm_payload[32], ntlm_payload[33], ntlm_payload[34], ntlm_payload[35]]) as usize;
                if dom_offset + dom_len <= ntlm_payload.len() {
                    domain = String::from_utf16_lossy(&to_u16_slice(&ntlm_payload[dom_offset..dom_offset + dom_len]));
                }
                
                // Read User
                let user_len = u16::from_le_bytes([ntlm_payload[36], ntlm_payload[37]]) as usize;
                let user_offset = u32::from_le_bytes([ntlm_payload[40], ntlm_payload[41], ntlm_payload[42], ntlm_payload[43]]) as usize;
                if user_offset + user_len <= ntlm_payload.len() {
                    user = String::from_utf16_lossy(&to_u16_slice(&ntlm_payload[user_offset..user_offset + user_len]));
                }
                
                // Read Workstation
                let work_len = u16::from_le_bytes([ntlm_payload[44], ntlm_payload[45]]) as usize;
                let work_offset = u32::from_le_bytes([ntlm_payload[48], ntlm_payload[49], ntlm_payload[50], ntlm_payload[51]]) as usize;
                if work_offset + work_len <= ntlm_payload.len() {
                    work = String::from_utf16_lossy(&to_u16_slice(&ntlm_payload[work_offset..work_offset + work_len]));
                }
            }
            ...
            if user.is_empty() && ntlm_payload.len() >= 64 {
                ...
                let user_offset = u32::from_le_bytes([ntlm_payload[40], ntlm_payload[41], ntlm_payload[42], ntlm_payload[43]]) as usize;
                if user_offset + user_len <= ntlm_payload.len() {
                    user = String::from_utf8_lossy(&ntlm_payload[user_offset..user_offset + user_len]).into_owned();
                }
            }
