...
fn security_header_type(payload: &[u8], offset: usize) -> &'static str {
    if offset + 12 <= payload.len() {
        let zero_seq = u32::from_le_bytes([
            payload[offset],
            payload[offset + 1],
            payload[offset + 2],
            payload[offset + 3],
        ]) == 0;
        let first_req = u32::from_le_bytes([
            payload[offset + 4],
            payload[offset + 5],
            payload[offset + 6],
            payload[offset + 7],
...
pub fn dissect_opc_ua_secure_conversation(
    ...
    let fallback = |s: String| DissectedResult {
        ...
        protocol: Protocol::OpcUaSecureConversation,
        summary: s,
    };
    if payload.len() < 12 {
    ...
    if !matches!(
        &payload[0..3],
        b"OPN" | b"MSG" | b"CLO" | b"HEL" | b"ACK" | b"ERR"
    ) {
    ...
    if body.len() < 8 {
        return fallback(format!(
            "OPC UA SecureConv {msg_type} (partial body, {size} bytes)"
        ));
    }
