...
pub fn dissect_nccl_allgather(
    ...
    if payload.len() >= 32 {
        ...
        let recvcount = u64::from_be_bytes([payload[12], payload[13], payload[14], payload[15], payload[16], payload[17], payload[18], payload[19]]);
        let _sendcount = u64::from_be_bytes([payload[20], payload[21], payload[22], payload[23], payload[24], payload[25], payload[26], payload[27]]);
        let tag = u64::from_be_bytes([payload[28], payload[29], payload[30], payload[31], payload[32], payload[33], payload[34], payload[35]]);
        let _ = nranks;
        let _ = recvcount;
        summary = format!("NCCL allgather algo={} rank={} tag={}",
            algo, rank, tag);
    } else {
        summary = "NCCL allgather (short frame)".into();
    }
    DissectedResult {
        src_addr: _src_ip,
        dst_addr: _dst_ip,
