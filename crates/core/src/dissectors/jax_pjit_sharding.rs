...
pub fn dissect_jax_pjit_sharding(
    ...
    if payload.len() >= 24 {
        ...
        let _partition_dim = u32::from_be_bytes([payload[12], payload[13], payload[14], payload[15]]);
        let _data_len = u64::from_be_bytes([payload[16], payload[17], payload[18], payload[19], payload[20], payload[21], payload[22], payload[23]]);
        ...
        summary = format!("JAX pjit type={} device={}/{} axis={}",
            msg_type, shard_idx, num_devices, axis_name);
    } else {
        summary = "JAX pjit (short frame)".into();
    }
    DissectedResult {
        src_addr: _src_ip,
        dst_addr: _dst_ip,
