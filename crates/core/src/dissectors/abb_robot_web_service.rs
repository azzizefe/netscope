...
pub fn dissect_abb_robot_web_service(
    ...
    let summary = if payload.len() >= 4 {
        ...
        let seq = u16::from_be_bytes([payload.get(2).copied().unwrap_or(0), payload.get(3).copied().unwrap_or(0)]);
        ...
        format!("ABB Robot Web Services — {type_name} RW{v} seq:{seq} ({len} bytes)", v = rw_version, len = payload.len())
    } else {
