...
pub fn dissect_nccl_allreduce(
    ...
    if payload.len() >= 32 {
        ...
        let count = u64::from_be_bytes([payload[4], payload[5], payload[6], payload[7], payload[8], payload[9], payload[10], payload[11]]);
        ...
        summary = format!("NCCL allreduce algo={} op={} count={} rank={}/{} tag={}",
            algo, op, count, rank, nranks, tag);
    } else {
        summary = "NCCL allreduce (short frame)".into();
    }
    DissectedResult {
        src_addr: _src_ip,
        dst_addr: _dst_ip,
