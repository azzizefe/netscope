...
fn uadp_extended_flags1(flags: u8) -> Vec<&'static str> {
    let mut f = Vec::new();
    if flags & 0x01 != 0 { f.push("PublisherId"); }
    if flags & 0x02 != 0 { f.push("WriterGroupId"); }
    if flags & 0x04 != 0 { f.push("WriterId"); }
    if flags & 0x08 != 0 { f.push("DataSetWriterId"); }
    if flags & 0x10 != 0 { f.push("PayloadHeader"); }
    if flags & 0x20 != 0 { f.push("MetaData"); }
    if flags & 0x40 != 0 { f.push("SeqNum"); }
    if flags & 0x80 != 0 { f.push("Chunk"); }
    f
}

fn uadp_extended_flags2(flags: u8) -> Vec<&'static str> {
    let mut f = Vec::new();
    if flags & 0x01 != 0 { f.push("SampleOffset"); }
    if flags & 0x02 != 0 { f.push("Duration"); }
    if flags & 0x04 != 0 { f.push("Picoseconds"); }
    if flags & 0x08 != 0 { f.push("DataClassId"); }
    if flags & 0x10 != 0 { f.push("PromotedFields"); }
    if flags & 0x20 != 0 { f.push("CoherentData"); }
    f
}

fn dataset_message_type(tag: u16) -> &'static str {
    match tag {
        0x0000 => "DataKeyValue",
        0x0001 => "RawData",
        0x0002 => "DataSet",
...
fn field_encoding(tag: u8) -> &'static str {
    match tag {
        0 => "Variant",
        1 => "RawData",
        2 => "DataValue",
...
pub fn dissect_opc_ua_pubsub_uadp_detail(
    ...
    let fallback = |s: String| DissectedResult {
        ...
        protocol: Protocol::OpcUaPubsubUadpDetail, summary: s,
    };
    if payload.len() < 4 {
    ...
    let tags1 = if has_ext1 { uadp_extended_flags1(ext_flags1) } else { vec![] };
    let tags2 = if has_ext2 { uadp_extended_flags2(ext_flags2) } else { vec![] };
