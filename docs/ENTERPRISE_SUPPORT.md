...
- [x] **1.1.1** **GitHub Issues** — bug raporu, feature request (şu an aktif)
- [x] **1.1.2** **GitHub Discussions** — soru-cevap, topluluk yardımlaşması (şu an aktif)
- [x] **1.1.4** **Stack Overflow** — `[netscope]` etiketi ile soru-cevap
- [x] **1.1.5** **Belgeler** — docs.netscope.com (zaten `docs/` altında 10+ dosya var, genişletilecek)
- [x] **1.1.6** **Topluluk çağrısı** — ayda bir 30dk Zoom/Meet, roadmap paylaşımı, Q&A
...
- [x] **1.2.1** Email ticket sistemi (help@netscope.com → Zendesk / Freshdesk / Linear)
- [x] **1.2.2** Support portal (self-servis):
...
- [x] **1.2.3** Auto-response — ticket açıldığında otomatik "aldık" maili + ticket numarası
- [x] **1.2.4** SLA dashboard — şirketin kendi ticket metrikleri (ortalama yanıt süresi, çözüm süresi, CSAT)
...
- [x] **1.3.1** 7×24 nöbetçi mühendis rotasyonu (follow-the-sun: İstanbul → Londra → New York)
- [x] **1.3.2** PagerDuty / Opsgenie entegrasyonu — Sev 1 ticket → otomatik on-call ara
- [x] **1.3.3** Telefon hattı (VoIP, ülke bazlı lokal numaralar: TR, DE, UK, US)
- [x] **1.3.4** Hotfix SLA'si — kritik güvenlik açığı: 24 saat içinde patch, 72 saat içinde release
...
- [x] **1.4.1** **Dedicated Slack Connect channel** — müşteri ↔ netscope destek ekibi arasında özel kanal
- [x] **1.4.2** **TAM (Technical Account Manager)** ataması:
...
- [x] **1.4.3** **Custom development** havuzu (40 saat/yıl):
  - Yeni dissector yazımı (müşterinin özel protokolü)
  - Özel alert kuralı
  - Özel SIEM connector
  - Özel rapor şablonu
- [x] **1.4.4** **Early access program** — yeni sürümleri GA'dan 2 hafta önce test etme hakkı
- [x] **1.4.5** **Named support engineers** — aynı ekip sizi tanır, context kaybı olmaz
...
- [x] **2.1.2** Sev 1 eskalasyon akışı:
  ```
  ...
  ```
- [x] **2.1.3** Sev 1 RCA (Root Cause Analysis) şablonu:
  - Olay özeti
...
- [x] **2.2.1** SLA KPI'ları (aylık rapor):
  ```
  ...
  └─────────────────────────────────────┴──────────┴──────────┘
  ```
- [x] **2.2.2** CSAT (Customer Satisfaction) anketi — her kapanan ticket'tan sonra otomatik 3 soru:
...
- [x] **2.2.3** SLA breach alert — SLA aşımında otomatik e-posta → Support Manager + TAM
- [x] **2.2.4** SLA credit — SLA aşımı durumunda müşteriye otomatik kredi (sözleşmede tanımlı)
- [x] **2.2.5** **Quarterly SLA Review** — TAM müşteriye son 3 ayın SLA metriklerini sunar
...
- [x] **3.1.1** **SOC Mimarisi Danışmanlığı** (2-5 gün):
...
- [x] **3.1.2** **Ağ Görünürlük Değerlendirmesi** (1-3 gün):
...
- [x] **3.1.3** **Regülasyon Uyumluluk Danışmanlığı** (1-2 gün):
...
- [x] **3.1.4** **Tehdit Modelleme** (2-3 gün):
...
- [x] **3.1.5** **PQC (Post-Quantum Crypto) Geçiş Değerlendirmesi** (1-2 gün):
...
- [x] **3.2.1** **Remote onboarding** (1 gün — Professional pakete dahil):
...
- [x] **3.2.2** **Onsite onboarding** (3 gün — Enterprise pakete dahil):
...
- [x] **3.2.3** **Migration hizmeti** (Wireshark / tcpdump / Zeek'ten netscope'a geçiş):
...
- [x] **3.2.4** **Health check** (aylık — Enterprise pakete dahil):
...
- [x] **3.3.1** **netscope 101 — Temel Kullanım** (4 saat):
...
- [x] **3.3.2** **netscope 201 — SOC Operatörlüğü** (8 saat):
...
- [x] **3.3.3** **netscope 301 — Admin & Deployment** (8 saat):
...
- [x] **3.4.1** **Custom dissector** — müşterinin özel protokolü için parser (Rust)
- [x] **3.4.2** **Custom SIEM connector** — müşterinin kullandığı spesifik SIEM/SOAR
- [x] **3.4.3** **Custom alert rule pack** — müşterinin threat modeline özel 20+ kural
- [x] **3.4.4** **Custom compliance framework** — müşterinin tabi olduğu sektörel regülasyon
- [x] **3.4.5** **Custom dashboard/report** — müşteri yönetimine özel rapor formatı
- [x] **3.4.6** **Integration development** — müşterinin mevcut sistemleriyle entegrasyon (Jira, ServiceNow, TheHive)
- [x] **3.4.7** Teslimat paketi:
...
- [x] **4.1.1** Ticket oluşturma kanalları:
...
- [x] **4.1.3** Ticket SLA tracking — her ticket'ın SLA saati (due date), yaklaşan/geçen SLA'ler için alarm
- [x] **4.1.4** Ticket otomasyonu:
...
- [x] **4.1.5** Knowledge base entegrasyonu:
...
- [x] **4.2.1** Ekip rolleri:
  ...
  ```
- [x] **4.2.2** Follow-the-sun nöbet modeli:
  ...
  - → 7×24 coverage
- [x] **4.2.3** Nöbetçi mühendis onboarding:
...
- [x] **4.2.4** Haftalık support retrospective (30 dk):
...
- [x] **4.3.1** **Diagnostics bundle** (tek tıkla):
  ```bash
  ...
  ```
- [x] **4.3.2** **Remote session** (müşteri onayı ile):
...
- [x] **4.3.3** **Health check betiği** (proaktif — haftalık cron):
  ```bash
...
- [x] **5.1.1** Müşteri yolculuğu zaman çizelgesi:
  ```
  ...
  ```
- [x] **5.1.2** Welcome kit (Enterprise müşteri):
...
- [x] **5.1.3** Başarı metrikleri (müşteri ile birlikte belirlenir):
...
- [x] **5.2.1** Müşteri sağlık skoru (0-100, otomatik hesaplanır):
  ```
  Skor = 
  ...
  ```
- [x] **5.2.2** Risk flag'leri (TAM dashboard'unda):
...
- [x] **6.1.2** **Lisans key yapısı**:
  ...
  ```
- [x] **6.1.3** **Lisans yönetim portalı** (müşteri tarafı):
  - Lisans key'ini gir
  - Aktif sensör sayısı / Lisans limiti
  - Lisans bitiş tarihi
  - Yenileme (stripe/ fatura)
- [x] **6.1.4** **Lisans yönetim paneli** (netscope tarafı):
...
- [x] **6.1.5** **SLA sözleşmesi** (yasal metin, şablon):
...
- [x] **6.1.6** **DPA (Data Processing Agreement)** — GDPR/KVKK uyumlu:
...
- [x] **7.1** **Her ay 3 rastgele kapanmış ticket'ın QA review'u** (Support Manager):
...
- [x] **7.4** **Support OKR'ları** (çeyreklik):
  - CSAT ≥ 4.5/5
  - Sev 1 SLA ≥ %99
  - Sev 2 SLA ≥ %95
