...
- **Benchmarks**: `cargo bench -p netscope-core` — `parse_throughput` (`dissect()` over 10k mixed packets), `pipeline_throughput` (ring + rayon stage), `filter_match`, `mem_usage`. Criterion rather than `cargo test`: a wall-clock assertion under parallel test load measures machine load, which is why the two tests that used to do this were `#[ignore]`d and so ran for nobody.
- **Smoke test**: `bench_corpus_dissects_without_failures` — the same 10k mixed packets must dissect with zero parse failures. Deterministic, runs on every `cargo test`.
